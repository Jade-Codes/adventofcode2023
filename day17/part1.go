...
func Part1() {
	...
	heatLoss := findLowestHeatLoss(matrix)

	fmt.Println("Day 17, Part 1:", heatLoss)
}

func findLowestHeatLoss(matrix [][]int) int {
	maxX := len(matrix[0]) - 1
	maxY := len(matrix) - 1
	...
	for priorityQueue.Len() > 0 {
		...
		locations := getNextLocations(x, y, current.location.direction, current.location.directionCount)
...
func getNextLocations(x int, y int, direction rune, currentCount int) []Location {
	locations := []Location{}
	if currentCount < 3 {
		locations = append(locations, getNextLocation(x, y, direction, currentCount+1))
	}
	switch direction {
	case north:
		locations = append(locations, getNextLocation(x, y, east, 1))
		locations = append(locations, getNextLocation(x, y, west, 1))
	case south:
		locations = append(locations, getNextLocation(x, y, east, 1))
		locations = append(locations, getNextLocation(x, y, west, 1))
	case east:
		locations = append(locations, getNextLocation(x, y, north, 1))
		locations = append(locations, getNextLocation(x, y, south, 1))
	case west:
		locations = append(locations, getNextLocation(x, y, north, 1))
		locations = append(locations, getNextLocation(x, y, south, 1))
	}
...
func getNextLocation(x int, y int, direction rune, count int) Location {
	switch direction {
	case north:
		y--
	case south:
